...
fname1 = 'HcChorus_090619_001_16C_0143.dat';
fname2 = 'HcChorus_090619_001_16C_0144.dat';
...
micarray1 = zeros(nblocks*nsamples_perblock, nchans);

fid = fopen(fname1, 'r', 'native');
...
for j = 1:nblocks
    for i = 1:nchans
        ...
        if (count ~= nsamples_perblock)
            ...
            micarray1((((j-1)*nsamples_perblock)+1):(j*nsamples_perblock),i) = x;
        end
    end
end
clear x
...
%micarray1 = uint32todouble(micarray1, minr, maxr, minv, maxv);


fid = fopen(fname2, 'r', 'native');
...
micarray = [ uint32todouble(micarray1, minr, maxr, minv, maxv); uint32todouble(micarray, minr, maxr, minv, maxv) ];
